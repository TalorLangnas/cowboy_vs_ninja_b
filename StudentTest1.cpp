...
TEST_SUITE("Battle related methods") {
    ...
    TEST_CASE("Cowboy shoot() and reload() methods") {
        ...
        shoot(1);
        CHECK(target->isAlive()); // Reloading when the magazine is n't empty shouldn't result in more than 6 bullets, the previous shoot should have no effect
        cowboy->reload();
        shoot(1);
...
TEST_SUITE("Battle simulations") {
    ...
    auto multi_attack = [](int n, Team &attacker, Team &defender) {
        for (int i = 0; i < n; i++) {
            cout << " -------------- multi attacl test ----------" << endl;
            cout << "iter = " << i << endl;
            if (defender.stillAlive()) {
                cout << "enter to  defender.stillAlive()"  << endl;
